...
module.exports = {
    ...
    Query: {
        ...
        allBooks: async (root, args, { currentUser }) => {
            ...
            if (args.genres) {
                const books = await Book.find({ user: currentUser }).populate(
                    'author'
                )
                let booksToReturn = []
        ...
        searchBooks: async (parent, args, { userLanguage, currentUser }) => {
            ...
            const books = await mapAsync(response.data.items, async book => {
                ...
                const author = book.volumeInfo.authors?.length
        ...
        searchBook: async (parent, args, { currentUser }) => {
            ...
            const author = bookData.volumeInfo.authors?.length
            ...
            const books = await mapAsync(nytimesBestSellers, async book => {
                ...
                if (searchBook) {
                    ...
                    return {
                        ...
                        author: searchBook.volumeInfo.authors[0],
    ...
    Mutation: {
        ...
        editUserProfilePhoto: async (root, args, { currentUser }) => {
            ...
            let profilePhoto = await processUpload(
                file,
                pathname,
                'profilePhoto'
            )
